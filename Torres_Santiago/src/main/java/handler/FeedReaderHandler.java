...
public class FeedReaderHandler {
    ...
    private static void handleKeyWordSearch(JavaRDD<Article> articleRDD, Scanner scan) {
        ...
        System.out.println("Escriba las palabras clave separadas por un espacio: \n");
        ...
        articleRDD.foreach(article -> {
            String cleanText = article.getText().toLowerCase().replace(",", "");
            List<String> textList = Arrays.asList(cleanText.split(" "));
            if (textList.containsAll(keywordsList)) {
                article.prettyPrint();
            }
        });
    }
            ...
            for (SingleSubscription sub : subscriptions.getSubscriptionsList()) {
                ...
                for (int i = 0; i < sub.getUrlParams().size(); i++) {
                    ...
                    if (sub.getUrlType().equals("nyt")) {
                        ...
                        Feed feed = rssParser.parseNYTFeed(rssParser
                                .parseXML(requester
                                        .getFeed(sub.getFeedToRequest(i))));
                        feedList.add(feed);

                    } else if (sub.getUrlType().equals("reddit")) {

                        Feed feed = jsonParser.parseRedditFeed(jsonParser
                                .parseJSON(requester
                                        .getFeed(sub.getFeedToRequest(i))));
                        feedList.add(feed);
